<script>
    ...
    let statsDB = {};
    let ogTags = {
        title: 'Meist geschaute Vods  • SchneileTV',
    ...
    async function fetchVods(p) {
        vods = await fetchApi(`/vods/?limit=48&page=${p}&order=viewcount,desc`);
        window.scrollTo(0, 0);
        return vods;
    }
</script>
    ...
    <div class="container">
        <div class="row mb-4">
            <div class="col-xs-12 col-md-8">
                <h1 class="display-4 fw-bolder p-0 m-0 align-self-center">Meist geschaute Vods</h1>
            </div>
            {#if vods}
